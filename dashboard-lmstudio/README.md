<!-- GENERATED by scripts/gen-readme.ts from readme.vars.yaml + the extension source. Do not edit; edit readme.vars.yaml and regenerate. -->

# @jpisgeek/dashboard-lmstudio

Normalizes scoped @jpisgeek/lmstudio endpoint, remote headless-daemon, and
request-probe output into dashboard bundle v1. Endpoint-down, token-rejected,
missing model capability, context exhaustion, output-token limit, and
reasoning-only empty output stay distinct. Token and timing values retain their
actual units and one probe is never presented as aggregate runtime accounting.

**Version** `2026.08.25.3` · **License** MIT · **Source**
https://github.com/jpisgeek/slog-bog/tree/main/dashboard-lmstudio

## Install

```
swamp extension pull @jpisgeek/dashboard-lmstudio
```

## Example

```yaml
# Require this report on any @jpisgeek/lmstudio model definition.
reports:
  require:
    - "@jpisgeek/dashboard-lmstudio"

# After a probe runs, pass its exact report artifact to the renderer:
globalArguments:
  bundles:
    - ${{ data.latest("lmstudio-probe", "report-jpisgeek-dashboard-lmstudio-json").attributes }}
```

## Caveats

Each report describes exactly one endpoint, daemon, or request-probe execution.
A completion bundle uses observed-traffic coverage with accountingScope
single-request and aggregateAccounting false. It does not measure requests that
bypass the probe. contextExhausted is the collector's documented heuristic:
finish_reason length before the requested output cap. A complete runtime ledger
requires instrumenting all traffic or a trustworthy aggregate runtime source.

## Security

This report performs no network, process, environment, or vault operations. It
reads only the data handles from the scoped LM Studio execution. The API token
and caller prompt are not present in those resources. Model identifiers, HTTP
status, timings, usage counts, and capability findings are operational metadata.
Classified collector errors can also retain a sanitized endpoint hostname or URL
without userinfo or query parameters; protect stored reports and rendered
dashboards accordingly.

See [SECURITY.md](https://github.com/jpisgeek/slog-bog/blob/main/SECURITY.md)
for the release gates every version passes before it reaches the registry.
